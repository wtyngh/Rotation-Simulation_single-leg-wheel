function data = Fn_rolling_simulation(landscpae_var, trajectory_table)
...
t_array = trajectory_table(1,:);  % t
theta_array = trajectory_table(2,:);
r_array = trajectory_table(3,:);

t_initial = t_array(1);
t_end = t_array(end);
theta_initial = theta_array(1);
theta_end = theta_array(end);
...
amp = landscpae_var.amp;
freq = landscpae_var.freq;
bias = landscpae_var.bias;

landscape_function = @(x) amp * sin( freq*x ) + bias ;
landscape_str = 'rough';


landscape_partition = landscape_function(x_partition);

f = fit(x_partition',landscape_partition','fourier2');
landscape_analysis = [mean(landscape_partition), std(landscape_partition), var(landscape_partition), f.w];


str_landscape_function = [num2str(amp),'*sin(',...
                          num2str(freq),'*x)+',...
                          num2str(bias)];

landscape_str_full = [landscape_str,', ',str_landscape_function];
...
if enable.plot_procedure == 0
    ...
    Analysis_3_txt = ['average speed = ',num2str(average_speed)] ;
    ...
    text( x_range(2) - 0.55 , y_range(2) - 0.13 , Analysis_3_txt ,'color', 'k', 'fontsize', 12);
...
data.landscape_analysis = landscape_analysis;
